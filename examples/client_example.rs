...
async fn main() -> Result<()> {
    ...
    let client1 = FlareIMClientBuilder::new("user1".to_string())
        .server_addresses(flare_im::client::config::ServerAddresses::new()
            .with_websocket_url("ws://127.0.0.1:4000".to_string()))
        .protocol_selection_mode(flare_im::client::config::ProtocolSelectionMode::Specific(TransportProtocol::WebSocket))
        .build()?;
    ...
    let client2 = FlareIMClientBuilder::new("user2".to_string())
        ...
        .protocol_selection_mode(flare_im::client::config::ProtocolSelectionMode::Specific(TransportProtocol::QUIC))
        .build()?;
    ...
    let client3 = FlareIMClientBuilder::new("user3".to_string())
        .server_addresses(flare_im::client::config::ServerAddresses::new()
            .with_websocket_url("ws://127.0.0.1:4000".to_string())
            .with_quic_url("quic://127.0.0.1:4010".to_string()))
        .protocol_selection_mode(flare_im::client::config::ProtocolSelectionMode::AutoRacing)
        .build()?;
