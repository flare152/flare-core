...
async fn main() -> Result<()> {
    ...
    let config = QuicConfig {
        bind_addr: "127.0.0.1:4433".parse::<SocketAddr>().unwrap(),
        enabled: true,
    ...
    println!("启动QUIC服务器在: {}", bind_addr);
    println!("使用token 'test_token' 进行认证");
    println!("注意: 需要有效的TLS证书文件 (certs/server.crt 和 certs/server.key)");
